...
public class TimerManager : MonoSingleton<TimerManager> {
	...
	public void OnInitializeTimersWithCustomControlButton() {
		InitializeTimers(Int32.Parse(_customTimeInput.text), (String.IsNullOrEmpty(_customIncrementInput.text) ? Int32.Parse(_customIncrementInput.text) : 0));
	}
	...
	public void OnInitializeTimersWithControlButton(string control) {
		...
		InitializeTimers(Int32.Parse(control.Substring(0, index)), Int32.Parse(control.Substring(index + 1)));
	}
	...
	public float[] GetTimes() {
		return new float[] {_whiteTimer.GetTime(), _blackTimer.GetTime()};
	}
