...
namespace TriDimensionalChess.UI {
	...
	public class MenuController : MonoSingleton<MenuController> {
		...
		public bool IsPageOnTopOfStack(Page page) => _pageStack.Count > 1 && page == _pageStack.Peek();
		...
		///</summary>
		public Page GetCurrentPage() => _pageStack.Peek();

		///<summary>
		///Sets the IP address in the IP address textbox
		///</summary>
		public void SetIPAddress() => _ipAddressText.text = $"IP Address: {Server.Instance.GetIPAddress()}";
